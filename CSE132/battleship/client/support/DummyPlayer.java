...
public class DummyPlayer extends ClientSetup implements MessageSender {
	...
	private static final double MESSAGEPROBABILITY = 0.025;
	...
	public void processInput(DataInputStream dis) {
		try {
			...
			final int playerNum = dis.readByte();
			...
			final int numRows = dis.readShort();
			final int numCols = dis.readShort();
			...
			//
			final MoveProvider mp = gmp.generateMoveProvider(numRows, numCols);
			...
			while (true) {
				...
				if (s.equals("PlayerTurn")) {
					...
					throw new Error("Bad message " + playerNum + " " + s);
				//
