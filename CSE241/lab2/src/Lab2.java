...
public class Lab2 {
     ...
     */
    private static void maskTable(StringTable table, String maskSeq, int matchLength) {
     ...
     * @param matchLength length of the matches
     */
    private static void findMatches(StringTable table, String corpusSeq, int matchLength) {
        for (int j = 0; j < corpusSeq.length() - matchLength + 1; j++) {
            ...
            if (rec != null) {
                for (int k = 0; k < rec.getPositionsCount(); k++) {
                    Terminal.println(j + " " + rec.getPosition(k) + " " + key);
                }
            }
        }
    }
